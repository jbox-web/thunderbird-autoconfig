...
SOURCE_FILE = src/thunderbird-autoconfig.cr
OUTPUT_DIR  = bin
OUTPUT_FILE = thunderbird-autoconfig
...
docker-image: ## Build local platform Docker image for local development
	docker build . -t thunderbird-autoconfig-dev:latest
...
install: ## Install thunderbird-autoconfig in $(INSTALL_DIR)
	$(sudo) cp $(OUTPUT_DIR)/$(OUTPUT_FILE) $(INSTALL_DIR)/thunderbird-autoconfig

uninstall: ## Uninstall thunderbird-autoconfig from $(INSTALL_DIR)
	$(sudo) rm -f $(INSTALL_DIR)/thunderbird-autoconfig
